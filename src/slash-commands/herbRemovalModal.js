...
const { herbRemoval, processHerbRemoval } = require('../controllers/processHerbRemoval');

module.exports = {
    run: async({ interaction}) => {
        const modal = new ModalBuilder({
            customId: `herbRemoval-${interaction.user.id}`,
            title: `Herb Removal`,
        });
        ...
        const filter = (interaction) => interaction.customId === `herbRemoval-${interaction.user.id}`;
        ...
        interaction
            ...
            .then(async(ModalSubmitInteraction) => {
                ...
                const header = `:herb: **Herb Storage Removal** :herb:`;

                try {
                    const result = await herbRemoval(time, name, clan, herb, amount);
                    if (!result.success) {
    ...
    data: {
        name: 'herb-removal',
        description: 'Removes herbs from the backend'
    }
}